...
#include <SoftwareSerial.h>
#include <MQ131.h>
#include <WiFiManager.h>  // Use WiFiManager for WiFi configuration
#include <EEPROM.h>       // Include EEPROM library to store configuration
...
// WiFi Module (ESP8266) Serial Communication
#define ESP_RX 10  // Connect to ESP8266 TX
#define ESP_TX 11  // Connect to ESP8266 RX
SoftwareSerial espSerial(ESP_RX, ESP_TX);  // Serial communication with ESP8266
...
void setup() {
  Serial.begin(115200);          // Initialize debug Serial
  espSerial.begin(115200);       // Initialize SoftwareSerial for ESP8266 communication
  dht.begin();                   // Initialize DHT22 sensor
  pmsSerial.begin(115200);       // Initialize PMS5003
  Wire.begin();                  // Initialize I2C bus
...
void connectWiFi() {
  espSerial.println("AT"); // Check if ESP8266 is responsive
  delay(1000);
